...
test.describe('Comprehensive E2E: Room & Form Validations', () => {
  test('Room Creation & Joining: Entering room code connects and joins room seamlessly', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(page.locator('[data-testid="room-code-display"]')).toHaveText('NONEXI', { timeout: 15000 });
  });

  test('Player Count Guard: START GAME button remains disabled with < 3 connected players', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P2GuardUser', { timeout: 15000 });
