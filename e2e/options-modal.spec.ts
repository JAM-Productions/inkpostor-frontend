...
test.describe('Options Modal E2E Suite', () => {
  test('Host can configure and save all game options and verify mode lock rules', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(page.locator('[data-testid="room-code-display"]')).toBeVisible({ timeout: 15000 });

    // 2. Open Options modal
    const openOptionsBtn = page.locator('button[aria-label*="Options"i], button[aria-label*="Opciones"i], button:has(svg.lucide-settings)').first();
    ...
    const time40Btn = page.locator('button[role="radio"]').filter({ hasText: /40/ }).first();
    await time40Btn.click();
    await expect(time40Btn).toHaveAttribute('aria-checked', 'true');

    // Toggle Unlimited Ink
    const inkToggle = page.locator('button[aria-label*="ink"i], button[aria-label*="tinta"i]').first();
    ...
    const decreaseAttemptsBtn = page.locator('button[aria-label*="decrease"i], button[aria-label*="disminuir"i]').first();
    await expect(decreaseAttemptsBtn).toBeEnabled();
    await decreaseAttemptsBtn.click();

    // Increase attempts (from 2 back to 3)
    const increaseAttemptsBtn = page.locator('button[aria-label*="increase"i], button[aria-label*="aumentar"i]').first();
    ...
    const nextModeBtn = page.locator('button[aria-label*="Next"i], button[aria-label*="Siguiente"i]').first();
    await nextModeBtn.click();

    // Check locked indicator appears
    const lockedNotice = page.locator('[data-testid="impostor-guess-unavailable"]');
