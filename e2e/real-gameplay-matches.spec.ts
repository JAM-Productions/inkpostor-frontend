...
test.describe('Real Gameplay Match Simulations', () => {
  test('Match 1: Crewmate Victory by Unanimous Voting with Canvas Pixel Data Verification', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match1', { timeout: 15000 });
  ...
  test('Match 2: Impostor Victory by Framing Innocent Crewmate', async ({ browser }) => {
    // 1. Setup 3 players & start game
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match2', { timeout: 15000 });
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (name !== innocentCrewmateName && innocentCrewmateName) {
        const voteTargetCard = page.locator('button[data-testid*="vote-card-"]').filter({ hasText: innocentCrewmateName }).first();
        await expect(voteTargetCard).toBeEnabled({ timeout: 10000 });
        await voteTargetCard.click();
      } else {
        const skipBtn = page.locator('[data-testid="skip-vote-btn"]');
    ...
    for (const page of pages) {
      await expect(page.locator('body')).toContainText(/Inkpostor Won|Defeated|Won|Result|Victoria|Derrota/i, { timeout: 15000 });
    }
  ...
  test('Match 3: In-Game Impostor Secret Word Clutch Victory', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match3', { timeout: 15000 });
    ...
    for (const page of pages) {
      ...
      if (text.includes('INKPOSTOR') || text.includes('Inkpostor')) {
        ...
        const wordMatch = text.match(/SECRET WORD:?\s*([A-Z0-9\s]+)/i) || text.match(/PALABRA SECRETA:?\s*([A-Z0-9\s]+)/i) || text.match(/WORD IS:?\s*([A-Z0-9\s]+)/i);
  ...
  test('Match 4: Impostor Final Chance Ejection Victory', async ({ browser }) => {
    // 1. Host enables Impostor Can Guess & starts game
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match4', { timeout: 15000 });
    ...
    if (await guessInput.isVisible() && secretWord) {
      ...
      const skipGuessBtn = impostorPage.locator('[data-testid="skip-guess-btn"]');
  ...
  test('Match 5: Custom Words Chaos Championship', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match5', { timeout: 15000 });
