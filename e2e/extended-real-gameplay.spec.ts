...
test.describe('Extended Real Gameplay Match Simulations', () => {
  test('Match 6: Multi-Round Tie-Vote Showdown in HOT_WORD Mode', async ({ browser }) => {
    ...
    const prevModeBtn = pageHost.locator('button[aria-label*="Previous"i], button[aria-label*="Anterior"i]').first();
    ...
    await expect(pageHost.locator('body')).toContainText('P4Match6', { timeout: 15000 });
    ...
    for (const page of pages) {
      await expect(page.locator('body')).toContainText(/Defeated|Won|Result|Victoria|Derrota/i, { timeout: 15000 });
    }
  ...
  test('Match 7: 4-Player Mid-Turn Vote-Kick Threshold Ejection', async ({ browser }) => {
    // 1. Host creates room
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P4Match7', { timeout: 15000 });
    ...
    for (const page of [pageHost, pageP2, pageP3]) {
      const suspectsBtn = page.locator('button:has(svg.lucide-user-round-search), button[aria-label*="suspect"i]').first();
      if (await suspectsBtn.isVisible()) {
        await suspectsBtn.click();
      }

      const voteKickTargetBtn = page.locator('[data-testid="vote-kick-btn"]').first();
    ...
    await expect(pageP4.locator('#player-name').or(pageP4.locator('body'))).toBeVisible({ timeout: 15000 });
  ...
  test('Match 8: Limited Ink Depletion & Canvas Drawing Mechanics', async ({ browser }) => {
    // 1. Host creates room
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    const openOptionsBtn = pageHost.locator('button:has(svg.lucide-settings)').first();
    await openOptionsBtn.click();
    const inkToggle = pageHost.locator('button[aria-label*="ink"i], button[aria-label*="tinta"i]').first();
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match8', { timeout: 15000 });
  ...
  test('Match 9: Rapid 3-Round Continuous Campaign', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match9', { timeout: 15000 });
  ...
  test('Match 10: Live Mid-Game i18n Language Toggle & State Sync', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match10', { timeout: 15000 });
    ...
    await expect(pageHost.locator('body')).toContainText(/Dibujando|Hecho|Alerta|Jugadores/i, { timeout: 10000 });
