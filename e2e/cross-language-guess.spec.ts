...
test.describe('High-Value Production Test: Cross-Language Impostor Secret Word Validation', () => {
  test('Impostor guesses secret word across localized languages and resolves game outcome', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3CrossLang', { timeout: 15000 });
    ...
    for (const page of pages) {
      await expect(page.locator('body')).toContainText(/Your turn|turno|Dibujando|Drawing|Won|Victoria|Defeated|Result/i, { timeout: 15000 });
    }
