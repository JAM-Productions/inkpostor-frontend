...
test.describe('System Resilience: Timer Expirations & Auto-Advance', () => {
  test('Turn timer expiration auto-advances turn and voting timer expiration auto-skips unsubmitted votes', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3TimerExp', { timeout: 15000 });
    ...
    for (const page of pages) {
      await expect(page.locator('body')).toContainText(/Nobody was ejected|Result|Victoria|Derrota/i, { timeout: 20000 });
    }
