...
test.describe("Production Qualification: Host Actions & Exit Game", () => {
  test("Host End Game: Host opens End Game modal mid-game and terminates match cleanly for all players", async ({
    browser,
  }) => {
    ...
    await expect(pageHost.locator("body")).toContainText("P3EndAction", {
      timeout: 15000,
    });
    ...
    const confirmEndBtn = pageHost.locator(
  ...
  test("Exit Game: Player exits match mid-game and returns to JoinScreen", async ({
    browser,
  }) => {
    // 1. Setup 3 players & start game
    const ctxHost = await browser.newContext();
    ...
    await expect(pageHost.locator("body")).toContainText("P3ExitAction", {
      timeout: 15000,
    });
    ...
    const exitBtn = pageP2
      .locator(
        'button[aria-label*="exit"i], button[aria-label*="salir"i], button:has(svg.lucide-log-out)',
      )
      .first();
    ...
    const confirmExitBtn = pageP2
      .locator('[data-testid="confirm-exit-game-button"]')
      .first();
