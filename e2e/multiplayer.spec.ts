...
test.describe('Multiplayer E2E Flow', () => {
  test('Host creates a room and Player 2 joins in real-time', async ({ browser }) => {
    ...
    const roomCodeElement = hostPage.locator('[data-testid="room-code-display"]');
    ...
    await expect(player2Page.locator('body')).toContainText('HostPlayer', { timeout: 15000 });
    ...
    await expect(hostPage.locator('body')).toContainText('PlayerTwo', { timeout: 15000 });
