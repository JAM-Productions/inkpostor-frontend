...
test.describe("Multi-Impostor ORIGINAL Spoken Mode E2E Suite", () => {
  test("5-player ORIGINAL spoken mode with 2 Inkpostors: Role Reveal ➔ ORDER_INFO ➔ Round 1 Voting ➔ ORDER_INFO ➔ Round 2 Voting & Victory", async ({
    browser,
  }) => {
    // 1. Setup 5 Players
    ...
    // 2. Options: Select ORIGINAL mode and set 2 Inkpostors
    ...
    // Select ORIGINAL mode via carousel indicator
    const originalDot = hostPage
      .locator(
        '[data-testid="game-mode-carousel"] button[aria-label*="Original"i]',
      )
      .first();
    ...
    // Verify fellow Inkpostor name is revealed to both Inkpostors
    ...
    // All proceed to ORDER_INFO phase
    for (const item of pages) {
      const proceedBtn = item.page
        .locator(
          '[data-testid="proceed-to-drawing-btn"], button:has-text("Continue"), button:has-text("Continuar")',
        )
        .first();
    ...
    // Helper to confirm ORDER_INFO phase and advance to VOTING
    const confirmOrderInfoPhase = async () => {
      for (const item of pages) {
        const confirmOrderBtn = item.page
          .locator(
            '[data-testid="confirm-order-btn"], button:has-text("Ready"), button:has-text("Listo"), button:has-text("Començar")',
          )
          .first();
        if (
          await confirmOrderBtn.isVisible({ timeout: 5000 }).catch(() => false)
        ) {
    ...
    // 5. Round 1: ORDER_INFO ➔ VOTING
    await confirmOrderInfoPhase();
    ...
    // Round 1 Voting: Everyone votes for imp1, imp1 votes skip
    for (const item of pages) {
    ...
    // 6. Round 2: Next Round ➔ ORDER_INFO phase in ORIGINAL mode
    for (const item of pages) {
    ...
    // Confirm ORDER_INFO for Round 2
    await confirmOrderInfoPhase();
