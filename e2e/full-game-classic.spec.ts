...
test.describe('Deep E2E: Full CLASSIC Game Loop & Play Again', () => {
  test('Executes complete game: Lobby ➔ Role Reveal ➔ Drawing Turns ➔ Voting ➔ Ejection Results ➔ Play Again', async ({ browser }) => {
    // 1. Setup 3 Players
    ...
    await expect(pageHost.locator('body')).toContainText('PlayerThreeFull', { timeout: 15000 });
    ...
    for (const page of [pageHost, pageP2]) {
      const voteTargetCard = page.locator('button[data-testid*="vote-card-"]').filter({ hasText: 'PlayerThreeFull' }).first();
