...
test.describe("Backend Docs Edge Cases & Parity", () => {
  test("Lobby Host Kick: Host kicks player from LOBBY, target is removed and returned to JoinScreen", async ({
    browser,
  }) => {
    // 1. Host creates room
    ...
    await expect(pageHost.locator("body")).toContainText("P2LobbyKick", {
      timeout: 15000,
    });

    // 3. Host kicks P2LobbyKick
    const kickBtn = pageHost
      .locator(
        'button[aria-label*="P2LobbyKick"i], button[data-testid*="lobby-kick-btn-"]',
      )
      .first();
    await expect(kickBtn).toBeVisible({ timeout: 10000 });
    await kickBtn.click();

    const confirmKickBtn = pageHost
      .locator(
        'button[aria-label*="confirm"i], button[data-testid*="confirm-lobby-kick-btn-"]',
      )
      .first();
    ...
    await expect(pageP2.locator("#player-name")).toBeVisible({
      timeout: 15000,
    });
  ...
  test("VOTING Last Voter Disconnect: Disconnection of last pending voter resolves voting phase immediately", async ({
    browser,
  }) => {
    ...
    await expect(pageHost.locator("body")).toContainText("P3VoterDis", {
      timeout: 15000,
    });
  ...
  test("RESULTS Last Unconfirmed Player Disconnect: Disconnection of last unconfirmed player starts Next Round automatically", async ({
    browser,
  }) => {
    // 1. Setup 3 players & start game
    const ctxHost = await browser.newContext();
    ...
    await expect(pageHost.locator("body")).toContainText("P3NextRndDis", {
      timeout: 15000,
    });
