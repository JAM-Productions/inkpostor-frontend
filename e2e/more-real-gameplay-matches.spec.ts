...
test.describe('More Real Gameplay Match Simulations (Matches 11-15)', () => {
  test('Match 11: 5-Player Chaos Match with Split Vote Tie', async ({ browser }) => {
    ...
    const names = ['HostMatch11', 'P2Match11', 'P3Match11', 'P4Match11', 'P5Match11'];
    ...
    await expect(pageHost.locator('body')).toContainText('P5Match11', { timeout: 15000 });
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      if (i < 2) {
        const card = page.locator('button[data-testid*="vote-card-"]').filter({ hasText: 'P2Match11' }).first();
        if (await card.isEnabled()) await card.click();
        else await page.locator('[data-testid="skip-vote-btn"]').click();
      } else if (i < 4) {
        const card = page.locator('button[data-testid*="vote-card-"]').filter({ hasText: 'P3Match11' }).first();
        if (await card.isEnabled()) await card.click();
        else await page.locator('[data-testid="skip-vote-btn"]').click();
      } else {
  ...
  test('Match 12: Rapid Emergency Alert & Mid-Voting Disconnect', async ({ browser }) => {
    // Setup 3 players
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match12', { timeout: 15000 });
    ...
    for (const page of pages) {
      ...
      if (await alertBtn.isVisible()) {
        ...
        const confirmBtn = page.locator('button', { hasText: /Confirm|Confirmar/i }).first();
  ...
  test('Match 13: Color Palette Switcher & Eraser Tool Drawing Sync', async ({ browser }) => {
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match13', { timeout: 15000 });
  ...
  test('Match 14: Mid-Lobby Options Config WebSocket Sync', async ({ browser }) => {
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P2Match14', { timeout: 15000 });

    await ctxHost.close();
    await ctxP2.close();
  });

  test('Match 15: 4-Round Continuous Championship Campaign', async ({ browser }) => {
    const ctxHost = await browser.newContext();
    const pageHost = await ctxHost.newPage();
    await pageHost.goto('/');
    ...
    await expect(pageHost.locator('body')).toContainText('P3Match15', { timeout: 15000 });
