...
test.describe("System Limit Guard: Room Capacity Limit (10 Players)", () => {
  test("10 players fill lobby and 11th player join attempt is handled gracefully", async ({
    browser,
  }) => {
    // 1. Host creates room
    ...
    await expect(pageHost.locator("body")).toContainText("Player10Cap", {
      timeout: 15000,
    });
    ...
    await expect(pageP11.locator("#player-name")).toBeVisible({
      timeout: 15000,
    });
