...
test.describe('Deep E2E: Multi-Round HOT_WORD Game Loop', () => {
  test('HOT_WORD Mode: Round 1 tie vote ➔ Next Round ➔ WORD_REVEAL with new word ➔ Round 2 DRAWING', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3HotWord', { timeout: 15000 });
    ...
    for (const page of pages) {
      await expect(page.locator('body')).toContainText(/Nobody was ejected|Nadie ha sido expulsado|Result/i, { timeout: 15000 });
    }
