...
test.describe('Deep E2E: Impostor Final Guess Feature Flow', () => {
  test('Impostor Can Guess enabled ➔ Impostor ejected ➔ IMPOSTOR_GUESS phase ➔ Skip ➔ Crewmates Win', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3GuessMode', { timeout: 15000 });
    ...
    for (const page of [pageHost, pageP2]) {
      const voteTargetCard = page.locator('button[data-testid*="vote-card-"]').filter({ hasText: 'P3GuessMode' }).first();
