...
test.describe('High-Value Production Test: Canvas Undo Stack & Clear Pixel Synchronization', () => {
  test('Active drawer draws strokes, triggers Undo twice, and verifies 100% pixel match across clients', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3UndoSync', { timeout: 15000 });
