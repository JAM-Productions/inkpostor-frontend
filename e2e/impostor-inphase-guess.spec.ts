...
test.describe("Production Qualification: In-Phase Impostor Guesses", () => {
  test("In-Phase Correct Secret Word Guess triggers instant Inkpostor Win", async ({
    browser,
  }) => {
    ...
    await expect(pageHost.locator("body")).toContainText("P3InPhase", {
      timeout: 15000,
    });
    ...
    for (const page of pages) {
      ...
      if (cardText.includes("INKPOSTOR")) {
        ...
        const wordMatch =
          cardText.match(/SECRET WORD:?\s*([A-Z0-9\s]+)/i) ||
          cardText.match(/PALABRA SECRETA:?\s*([A-Z0-9\s]+)/i);
