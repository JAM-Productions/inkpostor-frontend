...
test.describe('Comprehensive E2E: Suspect Marker System', () => {
  test('Mark player as Suspect during DRAWING phase and verify suspect highlight in VOTING phase', async ({ browser }) => {
    ...
    await expect(pageHost.locator('body')).toContainText('P3SusMarker', { timeout: 15000 });
    ...
    const playersBtnP2 = pageP2.locator('button[aria-label*="Players"i], button[aria-label*="Jugadores"i]');
    if (!(await playersBtnP2.isVisible())) {
      nonTurnPage = pageHost;
    }

    const playersBtn = nonTurnPage.locator('button[aria-label*="Players"i], button[aria-label*="Jugadores"i]');
    ...
    const susToggleBtn = nonTurnPage.locator('button', { hasText: 'P3SusMarker' }).first();
    ...
    await expect(nonTurnPage.locator('body')).toContainText(/Suspect|Sospechoso/i, { timeout: 15000 });
