...
test.describe('System Security: Non-Host UI Permission Restrictions', () => {
  test('Non-host player cannot see START GAME button and options modal is read-only', async ({ browser }) => {
    // 1. Host creates room
    ...
    await expect(pageHost.locator('body')).toContainText('P2NonHost', { timeout: 15000 });
