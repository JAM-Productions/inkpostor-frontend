...
test.describe("Game Modes E2E Suite", () => {
  test("CLASSIC Mode: 3 players start game, reveal roles, and enter DRAWING phase", async ({
    browser,
  }) => {
    // Context 1: Host
    ...
    await expect(pageHost.locator("body")).toContainText("PlayerThree", {
      timeout: 15000,
    });
  ...
  test("CUSTOM_WORD Mode (Chaos): 3 players enter custom words and reach DRAWING phase", async ({
    browser,
  }) => {
    // Context 1: Host
    const ctxHost = await browser.newContext();
    ...
    await expect(pageHost.locator("body")).toContainText("P3Chaos", {
      timeout: 15000,
    });
